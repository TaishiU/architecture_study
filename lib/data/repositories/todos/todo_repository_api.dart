...
/// プロバイダ
final todoRepositoryAPIProvider = Provider<TodoRepositoryAPI>(
  (ref) => TodoRepositoryAPI(todoService: ref.read(todoServiceAPIProvider)),
);

/// APIリポジトリ実装クラス
class TodoRepositoryAPI implements TodoRepository {
  /// コンストラクタ
  TodoRepositoryAPI({required this.todoService});
  ...
  Future<Result<Todo>> fetchById({required int id}) async {
    try {
      ...
      logger.e('[TodoRepositoryAPI] $error');
      return Result.failure(error);
    }
  }
